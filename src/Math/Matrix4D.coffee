	...
	Matrix4D :
		class Matrix4D
			...
			setToLookAt: (direction, up) =>
				lVez = new Vector3D(direction.x, direction.y, direction.z)
				lVez.normalize()
				
				lVex = new Vector3D(direction.x, direction.y, direction.z)
				lVex.normalize().crossLocal(up.x, up.y, up.z).normalize()
				
				lVey = new Vector3D(lVex.x, lVex.y, lVex.z)
				lVey.crossLocal(lVez.x, lVez.y, lVez.z).normalize()
